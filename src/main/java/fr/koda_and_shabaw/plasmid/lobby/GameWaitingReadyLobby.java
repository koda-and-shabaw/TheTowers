package fr.koda_and_shabaw.plasmid.lobby;
...
public class GameWaitingReadyLobby {
    ...
    private Map<UUID, Boolean> playersAreReady;

    private GameWaitingReadyLobby(GameSpace gameSpace, PlayerConfig playerConfig, TowersMap map, BossBarWidget bar, SidebarWidget sidebar) {
        this.gameSpace = gameSpace;
        this.playerConfig = playerConfig;
        this.bar = bar;
        this.sidebar = sidebar;
        this.map = map;
        this.playersAreReady = new HashMap<>();
    }
     ...
     */
    public static GameWaitingReadyLobby addTo(GameActivity activity, PlayerConfig playerConfig, TowersMap map) {
        ...
        var lobby = new GameWaitingReadyLobby(activity.getGameSpace(), playerConfig, map, bar, sidebar);
    ...
    private void onTick() {
        ...
        this.gameSpace.getPlayers().forEach(player -> {
            if (player.getBlockPos().getX() == (int) this.map.ready().getX() &&
                    player.getBlockPos().getY() == (int) this.map.ready().getY() &&
                    player.getBlockPos().getZ() == (int) this.map.ready().getZ()) {

                for (Map.Entry<UUID, Boolean> entry : playersAreReady.entrySet()) {
                    if (entry.getKey().equals(player.getUuid()) && !entry.getValue()) {
                        playersAreReady.put(entry.getKey(), Boolean.valueOf(true));
                        this.gameSpace.getPlayers().sendMessage(Text.of(String.format("The player %s is ready", player.getEntityName())));
                    }
                }
            }
        });
    ...
    private void onAddPlayer(ServerPlayerEntity serverPlayerEntity) {
        System.out.println("Hello player");
        playersAreReady.put(serverPlayerEntity.getUuid(), Boolean.FALSE);
    }
    ...
    private long getTargetCountdownDuration() {
        ...
        if (this.gameSpace.getPlayers().size() >= 2) {
    ...
    private boolean isReady() {
        ...
        for (Map.Entry<UUID, Boolean> entry : playersAreReady.entrySet()) {
            if (!entry.getValue().booleanValue()) canStart = false;
    ...
    private boolean isActiveFull() {
        ...
        if (this.gameSpace.getPlayers().size() >= server.getCurrentPlayerCount()) {
            System.out.println("Here is proc");
            return true;
        }
         */
